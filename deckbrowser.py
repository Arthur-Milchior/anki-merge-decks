...
def _rename(self, did, newName=None):
    ...
    try:
        if newName in self.mw.col.decks.allNames():
            merge = askUser(
                _("The deck %s already exists. Do you want to merge %s in it ?") % (newName, oldName))
            if merge:
...
def _dragDeckOnto(self, draggedDeckDid, ontoDeckDid):
    ontoDeckName = self.mw.col.decks.newNameForDragAndDrop(
        draggedDeckDid, ontoDeckDid)
